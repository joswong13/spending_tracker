...
class NewMonth {
  NewMonth._privateConstructor();

  static final NewMonth getInstance = NewMonth._privateConstructor();
...
class StaticMonthModel {
  //Given a date and the month object, sets the month object
  static void setDate(DateTime date, NewMonth month) {
    //sets the month date
    month.date = date;
...
//Core Functions----------------------------------------------------------------------------------------------
  ...
  ///Gets the first day of the month (Mo = 1, Tu = 2, We = 3, Th = 4, Fr = 5, Sa = 6, or Su = 7 as an int).
  ///Gets the last day of the month as an int.
  static List<int> _calculateMonthObject(DateTime date) {
    List<int> firstAndLastDay = new List(2);
    DateTime beginningOfMonth = DateTime.utc(date.year, date.month, 1);
    DateTime endOfMonth = DateTime.utc(date.year, date.month + 1, 0);
    firstAndLastDay[0] = beginningOfMonth.weekday;
  ...
  static int _calculatePreviousMonthWeekStart(int firstWeekdayOfMonth) {
    return (firstWeekdayOfMonth * -1) + 1;
  }

  ///Builds an list of DateTime objects for each week and then adds it to _monthlyDateArray.

  static void _createDateArray(int dayCounter, int numDaysInMonth, DateTime date, NewMonth month) {
  ...
  ///Sets the current week for displaying in WeeklyView.
  static int _setCurrentWeek(List<List<DateTime>> monthlyDateArray, DateTime date) {
...
//EXTERNAL FCN----------------------------------------------------------------------------------------------
  ...
  ///Changes the date but will not rebuild the monthly calendar.
  static void setCurrentWeekIndex(DateTime date, NewMonth month) {
    month.date = date;
    _setCurrentWeek(month.monthlyDateArray, month.date);
  }
}
